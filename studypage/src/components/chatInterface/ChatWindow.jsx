...
// export default ChatWindow;
  ...
  const ChatWindow = ({ activeUser, teacher, pic, isInModal, teach,iscurrentUser }) => {
    ...
    const fetchMessages = async (pageNum = 1, append = false) => {
      ...
      try {
        const response = await fetch(
          `https://studypage-76hu.onrender.com/conversations/${chatActiveUser.conversationId}/messages?page=${pageNum}&limit=20`,
          {
            headers: { Authorization: `Bearer ${authToken}` },  
          }
        );
    ...
    const sendMessage = async (conversationId, content, attachments) => {
      try {
        ...
        const response = await fetch(
          `https://studypage-76hu.onrender.com/conversations/${conversationId}/messages`,
          {
            method: "POST",
            headers: { Authorization: `Bearer ${authToken}` },
            body: formData,
