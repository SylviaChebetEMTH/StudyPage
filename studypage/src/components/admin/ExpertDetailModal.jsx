...
const ExpertDetailModal = ({ expert, isOpen, onClose, currentUser }) => {
  ...
  const fetchComments = async () => {
    setLoadingComments(true);
    try {
      const response = await fetch(`https://studypage-76hu.onrender.com/experts/${expert.id}/comments`);
      const data = await response.json();
      setComments(data.comments || []);
    } catch (error) {
      console.error('Error fetching comments:', error);
  ...
  const handleSubmitComment = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`https://studypage-76hu.onrender.com/experts/${expert.id}/comments/${currentUser.id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${authToken}`
