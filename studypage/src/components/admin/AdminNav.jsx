...
const AdminPage = () => {
  ...
  // Define navigation links to eliminate duplication
  const navLinks = [
    { path: "/admin", label: "Dashboard" },
    { path: "/adminchat", label: "Messages", icon: faComments },
    { path: "/admin/projects", label: "My Projects" },
    { path: "/admin/balance", label: "Balance" },
    { path: "/admin/profile", label: "Settings" }
  ];

  const isAdminChatPage = location.pathname === '/adminchat';

  return (
    <div className="flex flex-col bg-gray-900">
      <TopNavBar isMenuOpen={isMenuOpen} setIsMenuOpen={setIsMenuOpen} />
      <AdminNavBar navLinks={navLinks} isAdminChatPage={isAdminChatPage} toggleSidebar={toggleSidebar} sidebarVisible={sidebarVisible} />
      {isAdminChatPage ? (
        <AdminChatLayout
        ...
        <div className=" text-white"> {/* Placeholder for other content pages */}</div>
      )}
    </div>
  );
};
...
const TopNavBar = ({ isMenuOpen, setIsMenuOpen }) => (
  <nav className="bg-gray-900 px-6 py-3 flex justify-between items-center border-b border-gray-700">
    <Link to="/" className="text-yellow-500 font-bold text-2xl">Study<span className="text-white">Page</span></Link>
...
// Admin Navigation Bar with optional chat sidebar toggle
const AdminNavBar = ({ navLinks, isAdminChatPage, toggleSidebar, sidebarVisible }) => (
  <nav className="bg-gray-800 border-b border-gray-700 px-6 py-2">
    <div className="flex justify-between items-center">
      <ul className="flex flex-wrap justify-center space-x-2 md:space-x-6">
        {navLinks.map(({ path, label, icon }) => (
          <NavLink key={path} to={path} end className={({ isActive }) => isActive ? "text-yellow-400 font-bold" : "hover:text-yellow-500"}>
      ...
      {isAdminChatPage && (
        <button onClick={toggleSidebar} className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded transition-colors duration-200">
          {sidebarVisible ? 'Hide Sidebar' : 'Show Sidebar'}
        </button>
      )}
    </div>
...
// Admin Chat Layout
const AdminChatLayout = ({ sidebarVisible, selectedConversation, setSelectedConversation }) => (
  <div className="flex flex-1 overflow-hidden">
    {/* Sidebar */}
    <div className={`${sidebarVisible ? 'block' : 'hidden'} md:block ${sidebarVisible ? 'w-full md:w-80' : 'w-0'} h-full flex-shrink-0 transition-all duration-300`}>
      <AdminSidebar onSelectConversation={setSelectedConversation} />
