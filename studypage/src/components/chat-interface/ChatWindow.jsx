...
const ChatWindow = ({ activeUser }) => {
  ...
  const sendMessage = async (conversationId, content, attachments) => {
    ...
    try {
      const response = await fetch(`https://studypage-76hu.onrender.com/conversations/${conversationId}/messages`, {
        method: 'POST',
        headers: {
      ...
      const fetchMessages = async () => {
        try {
          const response = await fetch(
            `https://studypage-76hu.onrender.com/conversations/${activeUser.conversationId}/messages`,
            {
