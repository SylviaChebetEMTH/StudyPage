...
const ProjectRequest = () => {
  ...
  /** Fetch Services for Project Type */
  useEffect(() => {
    if (selectedProjectType) {
      fetch(`${API_URL}/services?project_type=${selectedProjectType}`)
        .then((response) => response.json())
  ...
  const isFormValid = useMemo(() => {
    return !!(projectTitle && description && deadline && expertId && numberOfPages && attachments.length > 0);
  }, [projectTitle, description, deadline, expertId, numberOfPages, attachments]);
  ...
  const handleSuccess = useCallback(
    async (response) => {
      ...
      try {
        ...
        if (!projectResponse.ok) throw new Error("Failed to submit the project request.");
    ...
    [projectTitle, description, deadline, expertId, numberOfPages, selectedService, totalPrice, attachments, navigate, authToken]
  );
  ...
  const componentProps = useMemo(
    ...
    [email, totalPrice, isLoading, paymentRef, handleSuccess, isFormValid]
  );

  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      <h2 className="text-2xl font-bold mb-6">Request Expert</h2>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium">Project Type</label>
          <p className="bg-gray-100 p-2 rounded">{selectedProjectType}</p>
        </div>
        <div>
          <label className="block text-sm font-medium">Subject</label>
          <p className="bg-gray-100 p-2 rounded">{selectedSubject}</p>
        </div>
      </div>

      <label className="block text-sm font-medium mt-4">Project Title</label>
      <input type="text" value={projectTitle} onChange={(e) => setProjectTitle(e.target.value)} className="w-full p-2 border rounded" />
      ...
      <label className="block text-sm font-medium mt-4">Attachments</label>
      <input type="file" multiple onChange={handleFileChange} className="w-full p-2 border rounded" />
      <ul className="mt-2">{attachments.map((file, index) => <li key={index} className="text-sm">{file.name} <button onClick={() => removeAttachment(index)} className="text-red-500 ml-2">Remove</button></li>)}</ul>
